...
- 2002–2005: BSc in Mathematics, University of Genoa, Italy.
- 2005–2008: MSc in Statistics and Informatics, University of
Padova, Italy. Advisor: Chiara Romualdi.
- 2009–2012: PhD in Statistics, University of Padova, Italy. Advisor: Monica Chiogna.
- 2012–: Postdoc, Department of Statistics and Division of Biostatistics, University of California, Berkeley. Advisor: Terry Speed and Sandrine Dudoit.
...
## Preliminaries: install useful packages

```{r packages, eval=FALSE}
install.packages(c("magrittr", "RColorBrewer", 
                   "matrixStats", 
                   "Rtsne", "e1071"))
```
...
\centering
\includegraphics[width=\linewidth]{figures/pie_chart}
...
\centering
\includegraphics[width=.8\linewidth]{figures/fuck_grapefruit}
...
t-SNE is a non-linear dimensionality reduction technique proposed by @maaten2008visualizing.

\bigskip
The t-SNE algorithm comprises the following steps:

1. Compute the pairwise Euclidean distance between points in the high-dimensional space.
2. Transform the distance matrix into a similarity matrix using a _varying Gaussian kernel_.
    - The similarity between $X_j$ and $X_i$ is the joint probability that $X_i$ will choose $X_j$ as its neighbor (and vice versa).
3. Map the points into a _random low-dimensional space_.
4. Compute similarities in the low-dimensional space using a _t-distribution kernel_.
5. Use _gradient descent_ to minimize the Kullback-Leibler divergence between the high-dimensional and low-dimensional distributions.

## t-distributed stochastic neighbor embedding (t-SNE)

More formally, the similarity between $X_j$ and $X_i$ is defined as
$$
...
$$
min_{Y_1, \ldots, Y_n} \sum_i \sum_j p_{ij} \log \frac{p_{ij}}{q_{ij}}.
$$

## t-distributed stochastic neighbor embedding (t-SNE)

t-SNE has become extremely popular for the _visualization of single-cell RNA-seq data_.

\bigskip
However, one should keep in mind that _only local patterns are meaningful_ and large distances should not be given too much interpretation.
...
library(Rtsne)
set.seed(223)
tsne <- Rtsne(t(golub), perplexity = 5)
```

## Example

```{r tsne_golub2}
plot(tsne$Y, pch=19, col=colors[pheno], cex=2, 
     xlab="Dimension 1", ylab="Dimension 2")
legend("bottomright", levels(pheno), fill=colors)
```
...
\centering
\includegraphics[width=.8\linewidth]{figures/classification}
...
\bigskip
The following figures compare the _partitions_ produced by different classifiers based on the learning set ${\cal L}_n$: _linear discriminant analysis_ (LDA), _quadratic discriminant analysis_ (QDA), _naive Bayes_, _$k$-nearest-neighbor_ ($k$-NN), with $k=1,3,5$.
...
\bigskip
The _classification rule_ is to assign the observation to the population with the greatest posterior probability.
...
- _Penalized discriminant analysis_ (PDA). A penalized Mahalanobis distance is used to enforce smoothness.
...
2. Use the top 2, 4, ..., 20 genes to classify the leukemia samples with LDA.
...
\centering
\includegraphics[width=.8\linewidth]{figures/cross-val}
...
1. Find/Retrieve the most variable genes.
2. Use the top 2, 4, ..., 20 genes to classify leukemia samples
with LDA.
...
A common statistical inference problem is to _assign observational units to classes_ on the basis of explanatory variables describing these observations.

\bigskip
In _cluster analysis_ the _classes are unknown_ a priori and need to be "discovered" or "learned" from a learning set of _unlabeled observations_.

A.k.a. class discovery, unsupervised learning, unsupervised pattern recognition.

## Cluster analysis

_Clustering_ (unsupervised) is in some sense a more difficult
problem than class prediction (supervised).  Typically, the issues
that must be addressed for class prediction must also be addressed for
clustering.  In addition, one encounters the following complexities.
  
* The (number of) _classes are unknown a priori_, 
i.e., there is no learning set of labeled observations.
* Implicitly, one must have already selected the relevant _observational units/explanatory variables_, _standardization_, and _distance_ measures.
* Many algorithms that are appealing from an intuitive or theoretical point of view are _computationally complex_. 
* Often, only approximate solutions are available and reproducibility is an issue.

\bigskip
The goals can be _vague_: "Find some interesting and important clusters in my data."

## Distances

Inherent in cluster analysis is a notion of _similarity_ or _distance between observations_. 

\bigskip
Let $d: \RR^J \times \RR^J \rightarrow \RR^{+}$ denote a _distance function_ for pairs of $J$-dimensional real vectors and
let $D = (d(X_i,X_j): i,\, j=1,\dotsc,n)$ denote the corresponding $n \times n$ _pairwise distance matrix_ for the $n$ observations to be clustered.
...
Given a distance measure between individual observations, one must
often also define a measure of _distance between clusters of observations_.  There are a number of ways of measuring the _distance between two clusters_.

* _Single linkage._ The distance between two clusters is the _minimum_ of all pairwise distances between the members of the two clusters.
* _Average linkage._ The distance between two clusters is the _average_ of all pairwise distances between the members of the two clusters.
* _Complete linkage._ The distance between two clusters is the _maximum_ of all pairwise distances between the members of the two clusters.
* _Centroid distance._ The distance between two clusters is the _distance between their centroids_, where the definition of _centroid_ generally depends on the clustering method, e.g., average, median, medoid.

## Clustering procedures

We distinguish between the following two broad types of clustering procedures.

* _Hierarchical methods_  provide a hierarchy of clusters, from the smallest set, where all observations are in one cluster, through to the largest set, where each observation is in its own cluster.
* _Partitioning methods_ partition the observations into disjoint clusters and usually require specification of the number of clusters.
...
The observations are partitioned into _mutually exclusive and exhaustive clusters_.

\bigskip
The _number of clusters_ $K$ usually needs to be _prespecified_. 

\bigskip
Partitioning clustering algorithms often involve _optimizing some criterion/objective function_ by iteratively reallocating observations to clusters, 
e.g., minimize within-cluster sum-of-squares or sum of distances from nearest medoid.

## Partitioning methods: Examples

* _$k$-means_ and extensions to _fuzzy $k$-means_:  Observations assigned to cluster with the nearest mean, serving as cluster prototype (cf. Gaussian mixtures, principal component analysis).
* _Partitioning around medoids_ (PAM) [@KaufmanRousseeuw90].
* _Model-based clustering_, e.g., based on Gaussian mixture models [@FraleyRaftery02] [@McLachlanetal01].

## PAM: Partitioning around medoids

_Partitioning around medoids_ (PAM) of @KaufmanRousseeuw90 is a partitioning clustering method which operates on an $n \times n$ _distance matrix_, $D = (d(X_i,X_j))$, of pairwise distances between the $n$ observations to be clustered.

\bigskip
For a _prespecified number of clusters_ $K$, the PAM procedure involves searching for $K$ _medoids_, or representative observations, among the $n$ observations to be clustered.

\bigskip
Given a set of $K$ medoids, the $K$ _clusters_ are constructed by assigning each observation to the cluster with the nearest medoid.

\bigskip
PAM can be applied to _any distance_ matrix and can therefore accommodate general data types.
It tends to be more _robust_ than $k$-means.

## PAM: Partitioning around medoids

The goal is to find a set of $K$ medoids, ${\cal M}_K= \{M_k: k=1,\dotsc,K\} \subseteq \{X_i: i=1,\dotsc,n\}$,  which minimizes the _sum of the distances of each observation to its nearest medoid_.

\bigskip
Specifically, the _optimal set of medoids_ ${\cal M}_K$ is defined as
\begin{equation}
{\cal M}_K \equiv \argmin_{\{{\cal M}: {\cal M} \subseteq \{X_i: i=1,\dotsc,n\}, \, |{\cal M}|=K\}}\ \sum_{i=1}^n \min_{M \in {\cal M}} d(X_i,M).
\end{equation}

That is, the PAM algorithm seeks to minimize the _objective function_ $f: {\cal M} \rightarrow \RR$ defined by
\begin{equation}
f({\cal M}) \equiv  \sum_{i=1}^n \min_{M \in {\cal M}} d(X_i,M),
\end{equation}
where ${\cal M}\subseteq \{X_i: i=1,\dotsc,n\}$ and $|{\cal M}|=K$.

## PAM: Partitioning around medoids

For $n$ observations to be partitioned into $K$ clusters, there
are ${n \choose K} = \frac{n!}{K!(n-K)!}$ possible choices for the
$K$ medoids.  In practice, it is therefore infeasible to perform an
exhaustive search over all sets of $K$ medoids.

\bigskip
The PAM algorithm first seeks a good _initial set of medoids_ (`build` phase).
It then finds a _local optimum_ for the objective function $f$, that is, a set of medoids such that there is no single switch of an observation with a medoid that decreases the objective function (`swap` phase).

## Silhouette width

The _silhouette width_ of an observation $i$ is defined as
\begin{equation}
sil(i) \equiv \frac{b(i) - a(i)}{\max\{a(i),b(i)\}} \in [-1,1],
\end{equation}
where
$a(i)$ denotes the average distance between the $i$th observation and all
other observations in the cluster to which $i$ belongs and
$b(i)$ denotes the minimum average distance between the $i$th observation and observations in other clusters.

\bigskip
@Rousseeuw87 suggests using _silhouette plots_ to:
(i) _select the number of clusters_ $K$;
(ii) _assess how well individual observations are clustered_.

\bigskip
Intuitively, the _larger the silhouette widths_, the _better the clustering_.
...
table(pam(d, k=which.max(avgSil)+1)$clustering, pheno)
```

## Hierarchical clustering

Hierarchical clustering methods produce a _hierarchy_ or _tree_ of _nested clusters_.

\bigskip
Unlike partitioning methods, they _do not require a prespecified number of clusters_ $K$.

\bigskip
_Partitions into clusters_ can be obtained by _"cutting" the tree into "branches"_,  by specifying either a desired number of clusters or cut height.

\bigskip
We distinguish between

- _agglomerative_, i.e., bottom-up, hierarchical clustering;
- _divisive_, i.e., top-down, hierarchical clustering.
...
\bigskip
At each step, _merge the two nearest clusters_ using a measure of _between-cluster distance_.

\bigskip
R software: `hclust` function, `stats` package.

- _Advantages_: computational simplicity.
- _Disadvantages_: cannot correct for early errors.
...
\bigskip
At each step, _split clusters_ into two (or more) clusters.

\bigskip
Example: DIvisive ANAlysis (DIANA), `diana` R function in package `cluster`.

- _Advantages_: capture the main structure of the data.
- _Disadvantages_: cannot correct for early errors.

## Dendrograms

_Dendrograms_ are often used to visualize the nested sequence of
clusters resulting from hierarchical clustering.

\bigskip
While dendrograms are appealing because of their apparent ease of interpretation, they _can be misleading_.

\bigskip
Firstly, the dendrogram corresponding to a given hierarchical clustering
is _not unique_.
For each merge/split, one needs to specify which subtree or branch should go on the left and which on the right.
...
A second and perhaps less recognized shortcoming of dendrograms is
that they  _impose structure_ on the data, instead of revealing structure in these data.
...
plot(hc, main="Hierarchical clustering dendrogram", sub="Average linkage agglomeration, one-minus-correlation distance")
...
\bigskip
Some general features that improve the _robustness_ and _stability_ of the clustering procedures are:

- _Resampling-based clustering_ 
- _Sequential clustering_
- _Ensemble / consensus clustering_
...
1. Subsample the data, e.g. 70% of samples.
2. Find clusters on the subsample.
3. Create a co-clustering matrix D: 
...
Initially proposed by @tseng2005tight to cluster genes in microarray data.
...
A.k.a. _consensus clustering_. Given an underlying clustering strategy, e.g., $k$-means or PAM, we can 

1. Run the algorithm _varying the tuning parameters_ (e.g., number of variables $p$ or number of clusters $k$).
2. Collect the results from multiple clustering algorithms.
3. Compute a consensus by, e.g., majority vote.

As often happens in statistics, _averaging_ leads to more prediction power.
...
```{r resampling2}
colData(cl) <- DataFrame(pheno=pheno)
plotCoClustering(cl, sampleData="pheno")
```
...
\centering
\includegraphics[width=.35\linewidth]{figures/reproducible}
...
\centering
\includegraphics[width=.7\linewidth]{figures/reproducible2}

\source{Sidney Harris}

## Why should I bother?

If you ever asked one of these questions...

\centering
\large

\bigskip
_In what order do I run these scripts?_

\bigskip
_Where did I get these data?_

\bigskip
_Why did I filter out these genes?_

\bigskip
_How did I make that figure?_
